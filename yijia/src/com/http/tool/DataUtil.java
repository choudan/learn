...
//		baos.write(crc);
	...
	public synchronized static  String getByteOfHexHasBlank(byte[] data){
		...
		for(int i=0; i<data.length; i++){
			buf.append(" "+DataUtil.byte2Hex(data[i]));
		}
		return buf.toString();
	}
	 ...
	 * ************************计算数据长度******************************
	 */
	public synchronized static int[] calculateLen(byte[] data, int offset){
		int j = 0;
		int k = 0;
	
		ByteArrayOutputStream baos = new ByteArrayOutputStream();
		byte[] contentString = new byte[0];
		int len = byte2ArrayToInt(data, 3, 4);
		baos.write(data, offset, len-5);
		contentString = baos.toByteArray();
		baos.reset();
	...
	public static String stringToUnicode(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            int ch = (int) s.charAt(i);//Returns the character at the specified offset in this string.返回该字符的高位码，转成int型
...
//                str += "\\" + Integer.toHexString(ch);
	 ...
	 */ 
	  ...
	  * ****************************************************************************
	  */
	 public static String stringToGBK(String s) throws UnsupportedEncodingException{   
		 String str = "";    
		 for (int i = 0; i < s.length(); i++) 
		 {    
...
//			 str = str + " 0x" + shex.toUpperCase();  
	  ...
	  * ****************************************************************************
	  */
	 public static int getCountStringToGBK(String s) throws UnsupportedEncodingException{   
		 int i=0;
	 ...
	 * *
	 */ 
	 public static String GBK2eight(String str) throws UnsupportedEncodingException{
		 str=stringToGBK(str);
		 switch(str.length()){
		 case 2:
	 ...
	 * ************************把任意字节的数组转成int型整数******************
	 */
	public synchronized static int byte2ArrayToInt(byte[] data,int offset,int len) 
	{
		StringBuilder buf = new StringBuilder();
		for(int i=0; i<len; i++)
		{
			buf.append(DataUtil.byte2Hex(data[i+offset]));
		}
	...
	public static String hexCount2Dec(String str) { 
		...
		if(str.length()%2==0){
			for(int i=0;i<str.length();i=i+2){
				String dec = String.valueOf(Integer.parseInt(str.substring(i, i+2),16)); //基本类型			
				ab.append(dec);
			}			 
		}else{
    ...
    */  
	 ...
	 */ 
	public static String toTHex(String data) throws UnsupportedEncodingException 
	{
	...
	{
	    ByteArrayOutputStream baos = new ByteArrayOutputStream();
		String sendData = DataUtil.getByteOfHexHasBlank(baos.toByteArray());
		if (sendData.indexOf(" 5C ") != -1){
			sendData = sendData.replaceAll("5C", "5E 02");
		}
		if (sendData.indexOf(" 5E ") != -1){
		...
		sendData = sendData.replace(" ", "");
		data = DataUtil.hexStringToByte(sendData);                //十六进制字符串转换为Byte数组
