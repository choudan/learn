...
public class BluetoothLeService extends Service {
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status){
        	...
        	// write回调失败 status=128，    read回调失败status=128.  	status=0,毁掉成功；=9，数组超长
        	if (status == BluetoothGatt.GATT_SUCCESS) 
            {
//                broadcastUpdate(ACTION_DATA_AVAILABLE, characteristic);
    ...
    public void setCharacteristicNotification(BluetoothGattCharacteristic characteristic,
        ...
        // This is specific to Heart Rate Measurement.设置characteristic的描述值。
...
//    }  
    ...
    public BluetoothGattService getSupportedGattServices(UUID uuid) {
    	...
    	for(BluetoothGattService  mService:list){
    		if(mService.getUuid().equals(uuid)){
    ...
    public BluetoothGattCharacteristic getGattCharacteristic(UUID uuid,String serviceUUID, String characteristicUUID){
    	BluetoothGattCharacteristic characteristic=null;
    	BluetoothGattService mBluetoothGattService=getSupportedGattServices(uuid);
    	if(mBluetoothGattService!=null){
    		String gattServiceUUID = Long.toHexString(mBluetoothGattService.getUuid().getMostSignificantBits()).substring(0, 4).toUpperCase();
			for (BluetoothGattCharacteristic bluetoothGattCharacteristic : mBluetoothGattService.getCharacteristics()) {
				String characterUUID = Long.toHexString(bluetoothGattCharacteristic.getUuid().getMostSignificantBits()).substring(0, 4).toUpperCase();
