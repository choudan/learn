...
public class MyCaptchaRender extends Render {
	...
	private static String randomCodeKey = "YZM";  
	...
	public MyCaptchaRender(String randomKey,int width, int height, int count, boolean isCaseInsensitive) {
		if (StringKit.isBlank(randomKey)) throw new IllegalArgumentException("randomKey can not be blank");
		randomCodeKey = randomKey;
		if(width <=0 || height <=0 || count <=0){
			throw new IllegalArgumentException("Image width or height or count must be > 0");
			} 
	...
	public void render() {
		...
		vCode = encrypt(vCode);//md5加密
		Cookie cookied = new Cookie(randomCodeKey,vCode);
		cookied.setMaxAge(-1);
		cookied.setPath("/tyj_weixin_dingyuehao");
   ...
   result += ((hex.length() ==1 ) ? "0" : "") + hex;
	...
	public static boolean validate(Controller controller, String inputRandomCode) {
		System.out.println("qqqqqqqqqqq"+controller.getCookie(randomCodeKey));
